    ...
    {if NOT $entry_vars.wysiwyg}
        <div id="tools_entry" class="editor_toolbar">
            ...
            {if $entry_vars.wysiwyg_advanced}
                {if $iso2br}<input type="button" name="insX" value="NoBR" accesskey="x" style="font-style: italic" onclick="wrapSelection(document.forms['serendipityEntry']['serendipity[body]'],'<nl>','</nl>')">{/if}
            ...
            {else}
                {if $iso2br}<input type="button" value=" NoBR " onclick="serendipity_insBasic(document.forms['serendipityEntry']['serendipity[body]'], 'x')">{/if}
